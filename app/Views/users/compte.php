...
<div class="row">
    <?php foreach ($connectedUser->quizzQuestions() as $currentQuizzes) : ?>
        <div class="col-sm-4 mt-5">
            <div class="card border-0">
                <div class="card-body">
                    <a href="<?= $router->generate('quizzes_quiz',['id' => $currentQuizzes->getId()]) ?>">
                        <h3 class="card-title"><?= $currentQuizzes->getTitle() ?></h3>
                    </a>
                    <h4 class="card-text"><?= $currentQuizzes->getDescription() ?></h4>
                    <p class="card-author">by <?=$currentQuizzes->getHost()->getFirstName()?> <?=$currentQuizzes->getHost()->getLastName()?> </p>
                </div>
            </div>
        </div>
    <?php endforeach; ?>
